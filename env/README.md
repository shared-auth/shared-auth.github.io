...
This Astro/GitHub Pages repository is not a secret owner. `PUBLIC_DASHBOARD_URL` and `PUBLIC_DASHBOARD_URL_ALLOWLIST` are validated public build metadata and remain GitHub repository variables in the Pages workflow; the workflow must not decrypt SOPS files.
